...
- **Индекс эвикции** — файл **`_eviction_index.json`** в той же папке кеша. В нём только **evictable**-записи: `{ key, size, lastAccessed }`. Он используется для LRU: выбор файлов для вытеснения и порядок. Если индекс отсутствует или повреждён, он **пересобирается** по сканированию папки и чтению футеров файлов. Все операции с индексом сериализованы in-memory lock’ом.

При **чтении** (ответ по Range из кеша) **`lastAccessed`** обновляется **только в индексе** (в фоне через `event.waitUntil`), не в футере файла — так избегается одновременное чтение/запись одного файла OPFS (ошибки при перемотке). При **записи** нового файла текущее время пишется в футер и при evictable — в индекс добавляется запись.
