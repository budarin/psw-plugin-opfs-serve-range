...
- **opfsCacheControl** — plugin that on client messages (OPFS_REQUEST_DELETE_FROM_CACHE, OPFS_REQUEST_HAS_IN_CACHE, OPFS_REQUEST_LIST_CACHED_RESOURCES, OPFS_REQUEST_CLEAR_CACHE) performs the operation in OPFS, invalidates in-memory caches as needed, and responds with the matching OPFS_RESPONSE_*. Only registered folders are accepted. Client utilities listOpfsCachedResources, hasInOpfsCache, deleteFromOpfsCache, clearOpfsCache send these requests (2 s timeout).
...
- **Folder:** each plugin instance has **folderName** (required in options). All cache folders live under a **plugin root** **OPFS_PLUGIN_ROOT_DIR_NAME** (default **`.opfs-serve-range`**) in OPFS root: OPFS root → `.opfs-serve-range` → folderName. registerFolderConfig(folderName, config) at plugin creation; same folderName ⇒ same config (throw if different). getPluginRoot() returns the plugin root (cached); getOpfsDir(root, create, folderName) resolves folderName under the plugin root. Multiple plugins can share one folder (e.g. serve + network+cache + BF) with same folderName and consistent config.
...
- **OPFS_PLUGIN_ROOT_DIR_NAME** — plugin root folder name in OPFS (default `.opfs-serve-range`). **getPluginRoot()** — returns plugin root handle (cached). **getOpfsDir(root, create, folderName)** — resolves folderName under plugin root; result cached per folderName; on error (e.g. plugin root removed) calls **invalidateAllCachesAndPluginRoot()** and retries once. **invalidateAllCachesAndPluginRoot()** — clears plugin root cache and invalidates all registered folders’ caches (third level of error handling). **clearOpfsCache(folderName)** — invalidates caches and removes the folder under plugin root. **getMaxCacheFraction(folderName)** (effective, may be scaled), **getRangeCacheMaxSizeBytes(folderName)**, **getRangeCacheMaxEntries(folderName)**.
